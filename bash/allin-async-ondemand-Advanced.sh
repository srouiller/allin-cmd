#!/bin/sh
# allin-async-ondemand-Advanced.sh - 1.0
#
...
if [ $# -lt 3 ]; then                           # Parse the rest of the arguments
  ...
  echo
  echo "  Example $0 -v GcXfOzOP8GsBu7odeT1w3GnMedppEWvngCQ7Ef1IBMA= SHA256 'cn=Ueli Steck,o=Swiss Machine,c=CH'"
  echo "          $0 -v GcXfOzOP8GsBu7odeT1w3GnMedppEWvngCQ7Ef1IBMA= SHA256 'cn=Ueli Steck,o=Swiss Machine,c=CH' +41797425947"
  echo "          $0 -v GcXfOzOP8GsBu7odeT1w3GnMedppEWvngCQ7Ef1IBMA= SHA256 'cn=Ueli Steck,o=Swiss Machine,c=CH' +41797425947 'service.com: Sign?' EN"
  echo 
  exit 1
fi
     ...
     xmlns:ns5="urn:com:swisscom:dss:1.0:schema">
      <SignRequest Profile="urn:com:swisscom:dss:v1.0" RequestID="$REQUESTID">
        ...
        <OptionalInputs>
          ...
          <ns5:AddTimestamp Type="urn:ietf:rfc:3161"/>
        </OptionalInputs>
      </SignRequest>
    </ais:sign>
  </soap:Body>
...
if [ "$RC" = "0" -a "$http_code" -eq 200 ]; then
  ...
  RES_ID=$(sed -n -e 's/.*<ns3:ResponseID>\(.*\)<\/ns3:ResponseID>.*/\1/p' $SOAP_REQ.res)
  ...
  ## TODO
  echo "Requesti ID : $REQUESTID" 
  echo "Response ID to poll: $RES_ID"
