#!/bin/sh
# allin-tsa-json.sh - 1.0
#
# Generic script using curl to invoke Swisscom Allin service: TSA
# Dependencies: curl, openssl, base64, sed, date, tr
#
# Change Log:
...
for cmd in curl openssl base64 sed date tr; do
  hash $cmd &> /dev/null
...
http_code=$(curl --write-out '%{http_code}\n' $CURL_OPTIONS \
    --request POST --data-binary @$REQ --header "${HEADER_ACCEPT}" --header "${HEADER_CONTENT_TYPE}" \
...
# Debug details
if [ "$DEBUG" != "" ]; then
  [ -f "$REQ" ] && echo ">>> $REQ <<<" && cat $REQ
  [ -f "$REQ.log" ] && echo ">>> $REQ.log <<<" && cat $REQ.log
  [ -f "$REQ.res" ] && echo ">>> $REQ.res <<<" && cat $REQ.res
  echo ""
fi
