...
# allin-poll.sh - 1.0
#
# Generic script using curl to invoke Swisscom Allin service: Polling
# Dependencies: curl, openssl, base64, sed, date, xmllint
#
# Change Log:
#  1.0 19.11.2013: Initial version
...
if [ $# -lt 1 ]; then                           # Parse the rest of the arguments
  echo "Usage: $0 <args> ResponseID"
  echo "  -v         - verbose output"
  echo "  -d         - debug mode"
  echo "  ResponseID - ResponseID received in hash to SignResponse"
  echo
  echo "  Example $0 -v c8f7221a-be99-4100-9afe-0a1a415c5f30"
  echo
  exit 1
fi
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    <ais:pending xmlns="urn:oasis:names:tc:dss:1.0:core:schema"
     ...
     xmlns:poll="urn:oasis:names:tc:dss:1.0:profiles:asynchronousprocessing:1.0">
      <poll:PendingRequest Profile="urn:com:swisscom:dss:v1.0">
        <OptionalInputs>
          <ClaimedIdentity Format="urn:com:swisscom:dss:v1.0:entity">
            <Name>$AP_ID</Name>
          </ClaimedIdentity>
          <poll:ResponseID>$RESPONSEID</poll:ResponseID>
        </OptionalInputs>
      </poll:PendingRequest>
    </ais:pending>
  </soap:Body>
