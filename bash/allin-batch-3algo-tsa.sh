...
if [ $# -lt 2 ]; then                           # Parse the rest of the arguments
  ...
  echo
  echo "  Example $0 -v GcXfOzOP8GsBu7odeT1w3GnMedppEWvngCQ7Ef1IBMA= iKOFriAiFXE8d+T51HX4kCtV9HlTCSjotVaNdSnItq3d1sJ1Rwz+kYJ1dOMaETy+ LaawCtkt8G7585fzhQR9le0mVrAw2cnaEkFrJjLDRj+TXQmLNwsi8QbsuKqAnnQDAce9gw/X7zQGjs15DFnJiA=="
  echo 
  exit 1
fi
...
# TEST DIGEST ALGO
DIGEST_ALGOt[1]='http://www.w3.org/2001/04/xmldsig-more#sha224'
DIGEST_ALGOt[2]='http://www.w3.org/2001/04/xmlenc#sha256'
DIGEST_ALGOt[3]='http://www.w3.org/2001/04/xmldsig-more#sha384'
DIGEST_ALGOt[4]='http://www.w3.org/2001/04/xmlenc#sha512'

# Define the part of the Batch request
BATCH=""					# clearing the BATCH parameter
count=2					# add a counter for the Document batch ID
for i in "$@" 			# list the arguments (start at 1 -> could start at 2 w/ shift) 
do
	TMP="<DocumentHash ID=\""$count\""><dsig:DigestMethod Algorithm=\""${DIGEST_ALGOt[$count]}\""/><dsig:DigestValue>\""$i\""</dsig:DigestValue></DocumentHash>"
	count=$((count+1))
	BATCH=$BATCH$TMP	
done
